        ...
        try {
            ...
            await new Promise(async (resolve, reject) => {
                for (let leadData of data) {
                    const { name, body, contact, assignedTo, city, address, amount, product, detail1, detail2, detail3, detail4 } = leadData;
                    console.log(leadData);
                    const leadID = campID + "::" + generateRandomId(32);
                    const data={
                        name,body,contact,assignedTo,leadID,campID,orgID
                    }
                    if(city) data['city']=city;
                    if(address) data.address=address
                    if(amount) data.amount=amount
                    if(product) data.product=product
                    if(detail1) data.detail1=detail1
                    if(detail2) data.detail2=detail2
                    if(detail3) data.detail3=detail3
                    if(detail4) data.detail4=detail4
